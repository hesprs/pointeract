import { BaseModule, Click, Drag, MultitouchPanZoom, PreventDefault } from '@';
...
test('monkey test', () => {
	...
	expect(pos).toStrictEqual({ x: -100, y: -300 }); // Result: left 100, up 300
	expect(acc.scale > 1.7 && acc.scale < 1.72).toBe(true);
...
test('augmentation', () => {
	class Aug extends BaseModule {
		// oxlint-disable-next-line class-methods-use-this
		private readonly sample = () => {};
		readonly augmentation = { sample: this.sample };
	}
