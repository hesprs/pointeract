...
export default class Lubricator extends BaseModule {
	...
	constructor(...args: BaseArgs) {
		...
		Object.entries(toLubricate).forEach(([key, value]) => {
			...
			this.modifiers[key] = makeLubricate(states, value);
		});
	}
...
const reciprocalAbs = (num: number) => (num > 1 ? num : 1 / num);
...
const makeLubricate =
	(states: PerEventStates, options: PerEventOption) => (detail: GeneralDictionary) => {
		...
		accumulate(states.fields, options.fields, detail);
		return false;
	};
