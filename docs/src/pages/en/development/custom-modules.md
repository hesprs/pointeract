...
The functionalities of Pointeract are thoroughly achieved by modules. It also allows you to write your own modules with ease. You can find some module examples in [our GitHub repository](https://github.com/Hesprs/Pointeract/tree/main/packages/core/src/modules).
...
Then you can pass your custom events to `BaseModule` as a type parameter:

```TypeScript
class YourModule extends BaseModule<BaseOptions, CustomEvents> {
    onPointerMove = () => {
        this.dispatch('moveMove'); // no error
        this.dispatch('moveDetail', { x: 0, y: 0 }); // no error
    };
}
```
